...
CFLAGS += "-I. -Wall -I../include   -c -fPIC -O3 -shared"
SOURCES = "prussdrv.c"

do_compile () {
    ${CC} ${CFLAGS} ${SOURCES} -o prussdrv.o
    ${CC} -shared -o libprussdrv.so.${PV} prussdrv.o
}

do_install() {
    install -d ${D}${libdir}
    install -m 0755 ${S}/libprussdrv.so.${PV} ${D}${libdir}
    ln -s libprussdrv.so.${PV} ${D}${libdir}/libprussdrv.so.1
    ln -s libprussdrv.so.${PV} ${D}${libdir}/libprussdrv.so
}
